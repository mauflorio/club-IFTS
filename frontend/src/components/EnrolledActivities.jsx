...
export default function EnrolledActivities() {
  ...
  useEffect(() => {
    async function fetchEnrolledActivities() {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/usuario-actividades/${user.email}`);
        if (!response.ok) {
  ...
  const handleUnsubscribe = async (actividadId) => {
    const confirm = window.confirm("¿Estás seguro que deseas darte de baja de esta actividad?");
    if (!confirm) return;
      ...
      try {
        ...
        setLoading(true);
        const res = await fetch(`http://localhost:3001/api/usuario-actividades/${user.email}`);
        if (!res.ok) {
  ...
  return (
    <div>
      <h2>Actividades Inscriptas</h2>
      <ul>
        {activities.map((act) => (
          <li key={act.id} style={{ marginBottom: '1rem' }}>
            <h3>{act.nombre}</h3>
            <p><strong>Día y horario:</strong> {act.dia} - {act.horario}</p>
            <p><strong>Entrenador:</strong> {act.entrenador}</p>
            <button onClick={() => handleUnsubscribe(act.id)}>Dar de baja</button>
          </li>
        ))}
      </ul>
    </div>
