...
export default function RegistrationForm() {
  ...
  const validate = () => {
    const errs = {};
    if (!form.nombre.trim()) errs.nombre = "El campo Nombre es obligatorio.";
    if (!form.apellido.trim())
      errs.apellido = "El campo Apellido es obligatorio.";
    if (!form.dni.trim()) errs.dni = "El campo DNI es obligatorio.";
    ...
    if (!form.password) errs.password = "Debe ingresar una contraseña.";
    if (!form.confirmPassword)
      errs.confirmPassword = "Debe confirmar la contraseña.";
    if (form.password !== form.confirmPassword)
      errs.confirmPassword =
        "Las contraseñas ingresadas no coinciden. Vuelva a intentarlo.";
    return errs;
  };
  ...
  return (
    <section className="register-section">
      <h1 className="register-title">Registro de Socio</h1>
      ...
      <form onSubmit={handleSubmit} noValidate className="register-form">
        {[
          ...
          <div key={key} className="register-group">
            <label htmlFor={key} className="register-label">
              {label}
            </label>
            <input
              id={key}
        ...
        <button style={{ marginTop: '1rem' }} type="submit" className="register-button">
          Registrarse
        </button>
      </form>
